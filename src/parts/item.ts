...
export class Item extends MyObject3D {
    ...
    for(let i = 0; i < num; i++) {
      ...
      const m = new Mesh(
        ...
        new ShaderMaterial({
          ...
          side:DoubleSide,
          uniforms:{
            tDiffuse:{value:TexLoader.instance.get('./assets/img/sample.png')},
            rot:{value:0},
            fix:{value:1},
            rate:{value: 0},
            noise:{value: i == 0 ? 0 : Util.random(0.9, 1.25)},
            noise2:{value: new Vector3(Util.random(0, 2),Util.random(0, 0.5),Util.random(0, 0.8))},
  ...
  protected _update():void {
    ...
    const area = Math.max(sw,sh) * Func.val(0.25, 0.5)

    const s = Util.map(d, 2, 1, 0, area)
